"""Standalone small-angle polynomial transfer twin.

The production phase cap bounds each scaled transfer angle by 0.25.  This
prototype preserves the production grid, phase subdivision, assembly and tail
handoff, replacing only ``sin``/``cos`` in that certified branch by fixed
Taylor polynomials.  It is an experimental, non-bitwise path.
"""
...
def _scaled_step_poly(xh, yh, w, h):
    w2 = w * w
    if w2 >= 1.0:
        omega = math.sqrt(w2 - 1.0)
        q = omega * h
        q2 = q * q
        c = 1.0 + q2 * (-0.5 + q2 * (1.0 / 24.0 + q2 * (
            -1.0 / 720.0 + q2 * (1.0 / 40320.0 + q2 * (-1.0 / 3628800.0)))))
        sinc = 1.0 + q2 * (-1.0 / 6.0 + q2 * (1.0 / 120.0 + q2 * (
            -1.0 / 5040.0 + q2 * (1.0 / 362880.0 + q2 * (-1.0 / 39916800.0)))))
        si = h * sinc
    else:
...
def _small_angle_error():
    worst = 0.0
    for q in np.linspace(1.0e-8, 0.25, 1001):
        h = 0.005
        w = math.sqrt(1.0 + (q / h) ** 2)
        expected = FS.scaled_step(0.2, -0.1, math.log(w), h)
        actual = _scaled_step_poly(0.2, -0.1, w, h)
        worst = max(worst, abs(actual[0] - expected[0]), abs(actual[1] - expected[1]))
    return float(worst)


@njit(inline="always", cache=True)
def _phase_segment_poly(xh, yh, z_start, z_end, h_step, phase_max):
    ...
    h_sub = h_step / n_sub
    if n_sub == 1:
        return _scaled_step_poly(xh, yh, math.exp(z_mid), h_step)
    dz = (z_end - z_start) / n_sub
    for sub in range(n_sub):
        z_sub_mid = z_start + (sub + 0.5) * dz
        xh, yh = _scaled_step_poly(xh, yh, math.exp(z_sub_mid), h_sub)
    return xh, yh


@njit(parallel=True, cache=True)
def solve_kernel_poly(
...
def main():
    ...
    payload = {
        "candidate": "small_angle_polynomial_transfer",
        "commit": subprocess.check_output(["git", "rev-parse", "HEAD"], text=True).strip(),
        "production_unchanged": True,
        "local_abs_error": _small_angle_error(),
        "settings": vars(args),
        "cases": ([ _oracle_metrics(name, args.threads)
                    for name in (args.case or list(CASES)) ] if args.oracle else
                   [ _run_outer(name, args.repeats, args.threads)
                     for name in (args.case or list(CASES)) ] if args.outer else
                   [ _run_kernel(name, args.threads, args.repeats)
