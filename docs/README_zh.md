...
根目录 README 是面向使用者的主指南。当前只有一个用户可见的 `fast` 档位；历史 `plain-grid`、`production` 结果只适用于对应实验记录。阅读精度数值时要同时核对运行 commit 和覆盖范围：manifest 是较早验证运行的只读汇总，不代表当前 HEAD 已通过全参数空间认证。
...
| `benchmarks.md` | 当前固定资源全流程 profile 和带日期的性能对比 | 以报告范围为准 |
