...
- **速度**：fast 比 SageNet+ 快约 2.9–4.9 倍。相较 plain-grid，基准情景每步耗时由 `7.013` 降至 `5.994 ms`（少 14.5%，吞吐量高 17%）；低温情景由 `3.907` 降至 `3.682 ms`（少 5.8%，吞吐量高 6%）；高刚性情景由 `6.515` 升至 `6.652 ms`（多 2.1%，吞吐量低 2%）。
  按每秒有效样本数计，三个情景分别变化 +37% / -22% / -16%。速度结果只适用于本报告的机器、版本和线程设置。
