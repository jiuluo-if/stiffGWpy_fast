...
- [ ] 继续优化 DN/速度并决定后续阶段是否合并；已接受物理尾部匹配修正（gamma=1）、背景节点缓存和 formal kink 的 frequency-only preparation，四阶 Magnus/曲率子步、low-T 局部加密、直接删除第二次 full solve 及批量 sigma 采样已拒绝
- **Status:** in_progress
