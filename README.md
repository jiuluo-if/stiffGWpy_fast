...
Consolidated parameter-validation matrix (machine- and human-readable): `docs/parameter_validation/validation_results.json`, `validation_results.csv` and `parameter_validation_report.md` — regenerated by `python scripts/build_validation_matrix.py` (a read-only replay of the artifacts below; no physics is re-run).
