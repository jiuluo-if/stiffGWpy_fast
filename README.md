...
> **Single source of truth.**  Every number quoted below is read back from
> `docs/validation/validation_manifest.json` (generated by reading the committed
> validation artifacts).  The README never hard-copies validation numbers, so
> the README and the artifacts cannot drift.  `docs/` holds the canonical
...
- **Where it is NOT acceptable:** scientific results or MCMC.  Measured at 9
  ...
  artifact.
- **Runtime (this host, warm, 4 threads):** ≈0.37 s/point (median), p95 ≈0.43 s.
...
- **Parameter space (240 Sobol, production):** 212 ok / 28 explicit guard
  rejections; runtime median ≈5.3 s/point.
- **Runtime (this host, warm, 4 threads):** ≈3.7 s/point single (matched-z8
  artifact measure ≈4.1 s); the boundary of what the tail/grid limit allows.
...
Runtime (this host, warm, 4 threads) vs the LSODA anchor (z5, 18.6 s/point):

| | runtime/point | vs LSODA |
|---|---|---|
| plain-grid | ≈0.37 s | ≈50x faster (but NOT accuracy-certified) |
| transition-refine (matched z8) | ≈3.7–4.1 s | ≈4.5x faster (accuracy-certified) |
| reference (oracle) | ≈360 s | ≈1x (anchor only) |

Historical warm-`~1000x` plain-grid benchmark numbers from the LSODA era are
superseded (they hold only for the coarsest pre-fix grid at p95 ~7e-2 spectrum
error) and are archived in `docs/archive/`.  They are **not** a current claim.
...
* Integrated `DN_gw` relative <1e-4 is NOT met (median 4.3e-4); the residual is
  ...
  tuned away.
* `production` is not 100x faster than LSODA at matched accuracy (≈4.5x); the
  100x+ numbers are plain-grid only and are not accuracy-certified.
